...
# put some anomalies onto out function
y_noisy[12] -= 0.8
y_noisy[444] += 0.7
y_noisy[111] += 1.0
y_noisy[100:110] += 0.5

# put some silent anomalies
x_3x = x[700:720]
y_noisy[700:720] = np.abs(np.sin(3 * x_3x)).astype(np.float32) # 3 times more frequency
...
X_train = seq(y_clean, 10)
X_test = seq(y_noisy, 10)
...
class AnomalyAutoencoder(nn.Module):    # the class inherits from nn.Module is a model of the neural network
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(10, 5), # allows transforming data from one dimension to another, reducing dimensions enables extracting the most important features of a given dataset, which helps the model learn it
            ...
            nn.Linear(5, 3)    # shrinking the dimension to 4 values, which allows us to extract even more important features of the dataset
        )
    ...
    def forward(self, x):   # forward function is responsible for passing data through the network
        ...
        return decoded.view(x.size(0), 10, 1) # adding back the third dimension, so the output has the same shape as the input
...
# loss
# function tells us how well the model is doing, it compares the model's predictions with the actual data using mean square error
# mean square erros is the average squared difference between the estimated values and the true values, general measure of the qualitt of an estimator




# Silent anomaly
# A silent anomaly is a type of anomaly that is not easily detectable by traditional methods or algorithms.
# It is not caused clear value changes, bu once that are not easily noticeable.
# Autoencoder can reconstruct it well, but it is semantically incorrect e.g. the frequency or phase is different.

# In a first attempt (5000 epoches 0.4 loss), the silent anomaly for frequency and attenuator
# was not detected, only the phase shift was detected

# In the second second attempt (10000 epoches 0.0 loss) the anomaly for plus phase was alse detected,
# the only that was not not detected was attenuator anomaly

# When the attenuation is a bit different from the original value, the model can reconstruct it well

#I have tried to add another dimension to the tensor, that is derivative, but it does not work